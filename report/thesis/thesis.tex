...
Inspired by the fact that \gls{bananas} is built on an ensemble surrogate, we further explore incorporating Jackknife+-after-boostrap \cite{kim2020predictive}, a wrapper for predictive inference designed specifically for use with ensemble learners, into the calibration step. This approach is referred as \gls{btcp} in the following sections.
...
\label{sec:acq}
Finally, we describe in this section the acquisition functions and the aquisition optimization strategies.
\begin{description}
\item[Acquisition functions] We consider four commonly used acquisition functions  



\item[Acquisition optimization strategy] 
