...
\item[\gls{its}:] A sample is drawn from the distribution at random and its value is seen as the acquistion score for the candidate architecture $a$. Note that \gls{its} introduces most stochasticity into the search among the aforementioned acquisition functions.
