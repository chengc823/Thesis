...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37]{figs/init_size_cal_rate_03.png}
	\refstepcounter{figure}
	...
	\parbox{\linewidth}{
	 \vspace{1em}
 	  	{\small \textit{Figure \ref{fig: init_size_cal_rate_03}:} \gls{rmsce} (top) and validation accuracy (bottom) of SCP--ensemble methods with different initial evaluations when calibration set contains 30\% of total data.}}
 	}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/init_size_cal_rate_05.png}
	\refstepcounter{figure}
	...
	\\
 	\parbox{\linewidth}{
	\vspace{1em}
 	  	{\small \textit{Figure \ref{fig: init_size_cal_rate_05}:} \gls{rmsce} (top) and validation accuracy (bottom) of SCP--ensemble methods with different initial evaluations when calibration set contains 50\% of total data.}}
 	}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/scp_acq_func.png}
	\refstepcounter{figure}
...
\begin{figure}[H]
	...
	\\
 	\parbox{\linewidth}{
	\vspace{1em}
 	  	{\small \textit{Figure \ref{fig: scp_acq_search_strategy.png}:} \gls{rmsce} (top) and validation accuracy (bottom) of SCP using various acquisition search strategy, with 10 initial evaluations and calibration set containing 50\% of total data.}}
 	}
\end{figure}

Recall from the previous sub-section that \gls{its} in a mutation-based strategy gives the most stable performance. These observations suggest that while a certain degree of randomness seems to be beneficial, excessive randomness may hinder convergence and lead to inconsistent search performances. Likewise, prior work that evaluates Bayesian optimization in a low budget setting shows that when the search budget is low, local search is favored since an optimization method cannot sample the global search space sufficiently \cite{nomura2019simple}. Therefore, we consistently use a  mutation strategy that modifies 2 architectures for candidates sampling in the following experiments.
	...
	\item[Prediction Error Analysis:] Overall, the original BANANAS outperforms all ensemble-based calibrated search methods. In contrast to our prior expectation, SCP distinguishes as the best choice for performing calibration in this setting, while alternative methods that support more efficient data use	in general underperform in terms of both \gls{rmsce} and validation accuracies. The decline in performance may stem from various factors, possibly arising   either during the model training phase or in the calibration process. While the latter is difficult to verify, it is straitforward to perform a sanity check on the prediction accuracy for the ensemble model. To do this, we select a subset of evaluated methods that give the best performance in their respective group, and then compute their absolute prediction errors across the search duration. Specifically, in the context of \gls{cvcp} and \gls{btcp}, predictions are obtained by averaging across the outputs generated by all models fitted on different subsets of the data. 
	 ...
	 Figure \ref{fig: pred_err} visualizes the prediction errors for selected methods over time, where mean and standard deviation are aggregated over 150 epochs and 50 experiment trials. Notably, since we follow the setup of \cite{white2019bananas} and output 10 architectures in parallel, the prediction errors are smoothed using a rolling window of size 10.
		...
		\begin{figure}[H]
			...
			\refstepcounter{figure}
   			\addcontentsline{lof}{figure}{Figure~\thefigure: Smoothed Absolute Prediction Error over Search Time for Ensemble-based Methods}
			\label{fig: pred_err}
			\\
 			\parbox{\linewidth}{
			\vspace{0.2em}
...
\begin{description}[leftmargin=0cm, listparindent=\parindent]
	...
	\begin{figure}[H]
		...
		\refstepcounter{figure}
		\addcontentsline{lof}{figure}{Figure~\thefigure: Performances of \gls{scp} with Quantile Regressor}
	...
	\end{figure}	
 	...
 	\item[Performance for \gls{cvcp}:] Among all methods, \gls{cvcp} combined with a quantile regressor has the highest computational complexity, both in terms of runtime and the number of parameters to be fitted. In parallel to the previous experiments, we use a 3-fold cross-validation and begin with \gls{its} as the acquisition function. Initial evidences suggest that \gls{cvcp} combined with a quantile regressor is capable of producing highly accurate distribution estimates, achieving \gls{rmsce}s close to 0. Inspired by this observation, we consider that an acquisition function that relies more deterministically on the shape of the distribution, like \gls{ei}, may benefit more from an accurate distribution estimate. Therefore, we repeat the same experiments and use \gls{ei} for acquisition search. Additionally, we also visualize the performance of the SCP-quantile method to support exploring the effect of cross-validation. Fi
		...
		\begin{figure}[H]
			...
			\refstepcounter{figure}
			\addcontentsline{lof}{figure}{Figure~\thefigure: Performances of \gls{cvcp} with Quantile Regressor}
	...
	\begin{figure}[H]
		...
		\refstepcounter{figure}
		\addcontentsline{lof}{figure}{Figure~\thefigure: Performances of \gls{cvcp} with Quantile Regressors Trained on Varying Quantile Levels}
...
\addcontentsline{lot}{table}{Table~\thetable: Performance Summary of Top Methods in Each Category}
\parbox{\linewidth}{
...
The prediction error analysis for ensemble-based methods in Section \ref{sec: ensemble} suggests the strong performance of BANANAS is likely attributable to the accurate estimation of the distribution's location (e.g., mean). However, this analysis is not applicable to methods employing a quantile regressor. To facilitate general cross-method comparisons, we analyze the estimated conditional distribution, which is a common component possessed by all methods.  \cite{white2019bananas} finds that  acquisition function has limited impact on performance within the BANANAS framework. One possible explanation is that the Gaussian-like distribution estimate exhibits a relatively small variance. Intuitively, when a Gaussian distribution has a small variance, its values are concentrated narrowly within the domain. In such cases, the location of the estimated distribution plays a more critical role in determining the level of acquisition scores. To examine this hypothesis, we compute the standard deviation of the estimated  distributions at specific epochs across all methods. For each epoch in Figure \ref{fig: boxplot}, the results are collected over 50 experiment trials.
	...
	\begin{figure}[htbp]
		...
		\refstepcounter{figure}
		\addcontentsline{lof}{figure}{Figure~\thefigure: Box Plot of Standard Deviations of Estimated Distributions Across Methods at Selected Epochs}
		\label{fig: boxplot}
		\\
 		\parbox{\linewidth}{
 		\vspace{1em}
...
This study explores to enhance the uncertainty quantification method in the BANANAS framework by incorporating a \gls{cp} based calibration step, such that the uncertainty estimates can better align with the empirical likelihoods. Based on this idea, we propose the new framework BANANAS--CP and evaluate its performance on the benchmark dataset NAS-Bench-201. Although our novel method does not significantly outperform BANANAS, it provides valuable insight that contributes to a deeper understanding of BANANAS's capabilities:

\begin{itemize}
\item Conformal prediction can significantly reduce \gls{rmsce}, indicating that the estimated distributions, which are used for obtaining acquisition scores, generally exhibit well-calibrated shapes. Specifically, using a quantile regressor as the surrogate model in Bayesian optimization produces more accurate uncertainty estimates than an ensemble predictor, especially when more data is available.
 
\item However, the exact shape of estimated distributions is relatively unimportant in determining method's efficiency. Instead, their location (e.g., mean) and scale (e.g., standard deviation) play a more crucial role. 

\item Empirical data based calibration, especially when using with cross-validation, tends to yield heavy-tailed distributions and thus introduces additional noises and variations into the optimization procedure. A quantile regressor can better mitigate the increased variability than an ensemble model.
...
In summary, our evaluations suggest that uncertainty is not strongly correlated with  search performance and well-calibrated uncertainty estimates do not necessarily lead to increased efficiency. Instead, it is more important for the surrogate model to produce precise and reliable predictions. The surrogate model employed in BANANAS is simple yet effective, contributing to its strong performance. 


\section{Future Work}
\label{sec: future_work}
Our evaluations highlight several directions for further exploration. First, these \gls{cp}-based methods can be evaluated across different search spaces to gain a more comprehensive view how enhanced uncertainty estimates affect performance. For example, we observe in the experiments that methods that involve uncertainty calibration tend to exhibit lower performance variability, as evidenced by the reduced standard deviations across multiple trials. Thus, these \gls{cp}-based methods can be evaluated on the robustness dataset \cite{jung2023neural}, which may help assess whether uncertainty calibration enhances robustness to adversarial attacks and common image corruptions. Besides, since cell-based search spaces are constrained in terms of architectural expressiveness \cite{white2023survey}, evaluating these methods on other types of search spaces may reflect whether uncertainty estimation contributes to the discovery of novel architectures in different settings.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5 ]{figs/appendix/b1_quantile.png}
	\refstepcounter{figure}
	\label{fig: scp_num_quantile_20}
		\\
	\parbox{\linewidth}{
	\vspace{0.5em}
	{\small \textit{Figure \ref{fig: scp_num_quantile_20}}: Performances (validation accuracy) are evaluated for BANANAS and \gls{scp} methods with an ensemble predictor that form the linearly-interpolated distribution based on 10 or 20 quantile estimates, with a train-calibration splitting rate of 0.5. 
	}}
}	
\end{figure}	 	
...
This section presents results  analyzing how different acquisition search strategies affect the performance of various methods.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/appendix/b2_gaussian_acq_search_strategy.png}
	\refstepcounter{figure}
	\label{fig: b2_gaussian_acq_search_strategy}
		\\
	\parbox{\linewidth}{
	\vspace{0.5em}
	{\small \textit{Figure \ref{fig: b2_gaussian_acq_search_strategy}}: \gls{rmsce} and validation accuracy for BANANAS using various acquisition optimization strategies. All methods take 10 initial evaluations and use ITS as acquisition function. 
	}}
}	
\end{figure}	 	
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/appendix/b2_gaussian_arch_mutate.png}
	\refstepcounter{figure}
	\label{fig: b2_gaussian_arch_mutate}
		\\
	\parbox{\linewidth}{
	\vspace{0.5em}
	{\small \textit{Figure \ref{fig: b2_gaussian_arch_mutate}}: \gls{rmsce} and validation accuracy for BANANAS employing a mutation strategy that mutates different numbers of architectures. All methods take 10 initial evaluations and use ITS as acquisition function. 

	}}
}	
\end{figure}	

\subsection{SCP with Ensemble}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/appendix/b2_scp_arch_mutate.png}
	\refstepcounter{figure}
	\label{fig: b2_scp_arch_mutate}
		\\
	\parbox{\linewidth}{
	\vspace{0.5em}
	{\small \textit{Figure \ref{fig: b2_scp_arch_mutate}}: \gls{rmsce} and validation accuracy for SCP-Ensemble methods employing a mutation strategy that mutates different numbers of architectures. All methods take 10 initial evaluations and use ITS as acquisition function, with a train-calibration splitting rate of 0.5.
	}}
}	
\end{figure}

\subsection{CrossVal-CP with Ensemble}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/appendix/b2_cvcp.png}
	\refstepcounter{figure}
	\label{fig: b2_cvcp}
		\\
	\parbox{\linewidth}{
	\vspace{0.5em}
	{\small \textit{Figure \ref{fig: b2_cvcp}}: \gls{rmsce} and validation accuracy for CrossVal-CP ensemble based methods employing different acquisition optimization strategies. All methods take 10 initial evaluations and use a 3-fold cross-validation.
	}}
}	
\end{figure}

\subsection{SCP with Quantile Regressor}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/appendix/b2_scp_quantile.png}
	\refstepcounter{figure}
	\label{fig: b2_scp_quantile}
		\\
	\parbox{\linewidth}{
	\vspace{0.5em}
	{\small \textit{Figure \ref{fig: b2_scp_quantile}}: \gls{rmsce} and validation accuracy for SCP-Quantile methods that use different acquisition functions. Both methods take 10 initial evaluations and employ a quantile regressor trained on 10 quantile levels, with a train-calibration splitting rate of 0.5.
	}}
}	
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/appendix/b3_bootstrap_5.png}
	\refstepcounter{figure}
	\label{fig: b3_bootstrap_5}
		\\
	\parbox{\linewidth}{
	\vspace{0.5em}
	{\small \textit{Figure \ref{fig: b3_bootstrap_5}}: \gls{rmsce} and validation accuracy for \gls{btcp} methods that have 5 bootstraps and use different conformity score functions. Both methods take 10 initial evaluations and employ an ensemble predictor.	}}
}	
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.37 ]{figs/appendix/b3_bootstrap_10.png}
	\refstepcounter{figure}
	\label{fig: b3_bootstrap_10}
		\\
	\parbox{\linewidth}{
	\vspace{0.5em}
	{\small \textit{Figure \ref{fig: b3_bootstrap_10}}: \gls{rmsce} and validation accuracy for \gls{btcp} methods that have 10 bootstraps and use different conformity score functions. Both methods take 10 initial evaluations and employ an ensemble predictor.	}}
}	
\end{figure}
