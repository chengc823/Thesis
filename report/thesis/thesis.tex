...
Split Conformal Prediction (SCP), also known as Inductive Conformal Prediction (ICP), was first introduced in \cite{papadopoulos2002inductive} to address the aforementioned inefficient computation problem of OCP by replacing the transductive inference with inductive inference. SCP aims to learn a general prediction rule about the data using the training set. Then, this rule can be applied directly to obtain predictions when new data arrives in sequence, without re-using the training data repeatedly.
...
\label{sec:cp}
As reviewed in \ref{sec: reviewCP}, numerous conformal prediction algorithms have been proposed in recent research. This work identifies several approaches applicable in NAS for dividing a dataset into a training set and a calibration set. This section provides an overview of these splitting strategies, as well as the conformity scoring functions that are commonly used for regression problems.
\subsection{Split Conformal Prediction}
To begin, a natural choice for a baseline strategy is the Split Conformal Prediction (SCP), which is the simplest and most computationally efficient technique in the CP family.
