...
Another line of work strives to find suitable and efficient conformity score function. For example, \cite{romano2020classification} proposes a novel conformity score crafted for  multi-label classification tasks. While providing marginal coverage, this score is also fully adaptive to data complexities, thereby improves approximate conditional coverage. In parallel, \cite{romano2019conformalized} develops a conformity score that involves both upper and lower quantiles given by a specific coverage rate, enabling to combine the strengths of quantile regression and conformal prediction.
\vspace{0.5em} 

The underlying prediction algorithm also plays an important role in \gls{cp}, since the trained predictor determines the base from which intervals are constructed. 
