...
In this section, we present the empirical performance of methods using a quantile regressor for prediction.Specifically, we apply quantile regression in combination with two different \gls{cp} approaches: \gls{scp} and \gls{cvcp}.
...
\begin{description}[leftmargin=0cm, listparindent=\parindent]
	...
	\begin{figure}[H]
		...
		\\
 		\parbox{\linewidth}{
 		\vspace{1em}
  		{\small \textit{Figure \ref{fig: scp_quantile}:} Performances of \gls{scp} using a quantile regressor that fits on 10 quantile levels, with 10 initial evaluations and calibration set containing 50\% of all data points.}
 	 	}	
	\end{figure}	
 	 	
 	Figure \ref{fig: scp_quantile}
