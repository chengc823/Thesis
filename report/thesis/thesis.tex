...
\label{sec:scp}
To begin, a natural choice for a baseline calibration strategy is the Split Conformal Prediction (SCP). In this section, we start by introducing the standard SCP procedure, then proceed with the adaptions required to incorporate it into BANANAS-CP. The standard SCP can be summarized in Algorithm \ref{alg:SCP}:
\begin{algorithm}[htbp]
  \caption{Split Conformal Prediction}
  \label{alg:SCP}
  \begin{algorithmic}[1]
    \textbf{Input:} 
    A set of observations $\{(x_{i}, y_{i})\}_{i=1}^n$, a prediction algorithm $h(\cdot)$, a non-conformity measure $\myfunc{s(\cdot)}$, significance level $\alpha$, fraction of data assigned to the training set $p_{train}$, test data $x_{n+1}$. \vskip
	...
	\ENDFOR
	\STATE Return $\mathcal{C}_{\alpha}(x_{n+1}) \leftarrow \{y \,|\, s((h(x_{n+1}), y) \leq q\}$, where $q=\lceil(1-\alpha)(n_s+1)\rceil$ smallest of $S$, with $n_s = |S|$.
    \end{algorithmic}
\end{algorithm}
