...
\begin{figure}[H]
	...
	\refstepcounter{figure}
   	\addcontentsline{lof}{figure}{Figure~\thefigure: Impact of Acquisition Search Strategy on SCP Performance (Initial Size=10, Calibration Rate=0.5)}
...
\begin{description}[leftmargin=0cm, listparindent=\parindent]
 	\item[CrossVal-CP:] We apply a 3-fold cross-validation and evaluate performances using \gls{its} as acquisition function. However, the results are somewhat counterintuitive. 
	...
	\end{figure}
	The \gls{rmsce} in Figure \ref{fig: cvcp} (top) reflects that \gls{scp} dominates over \gls{cvcp} and consistently attains lower calibration error throughout the search time. As a result, \gls{cvcp} yields the weakest performance in uncertainty estimation among all evaluated approaches. Similar to previous findings with \gls{scp}, calibration error does not show strong correlation with the search performance. Uncalibrated BANANAS remains to be the top-performing method and result in highest mean validation accuracy. Moreover, cross-validation appears to introduce a rise in the standard deviation of final validation accuracies, especially when compared with \gls{scp}, reflecting reduced model stability across runs. 
	
	\item[Bootstrap-CP:] An important engineering design within this framework is the number of bootstrapped samples. \cite{pmlr-v139-xu21h} suggests that conformalization with 20 to 30 bootstrapped samples is sufficient to yield good and stable outputs. However, considering the limited data availability and potential computation time, we restrict our experiments to 5 and 10 bootstrap samples. The reason behind is that \gls{btcp}-based methods using 5 bootstrap samples provide a fair comparison to approaches employing an ensemble of \glspl{fnn}, while \gls{btcp} with 10 bootstraps aligns with methods based on a quantile regressor with 10 quantile levels, which we discuss in the next section.
	
	All ensemble-based approaches discussed so far employ a normalized absolute residual for conformity function, as specified by Equation \ref{cpscore}. In contrast, \gls{btcp} leverages the pure absolute residual \footnote{$|y_i - \hat{y}(x_i)|$} that disregards standard deviation estimate, because in a context with only either 5 or 10 bootstraps, sub-samples may not have sufficient observations for variance estimation and may thereby introduce biases. We also run experiments results  for this regular conformity score function (Appendix \ref{appendix: s3}). The empirical results confirm our expectations and show 
