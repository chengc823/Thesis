...
% Add packages
\usepackage[colorlinks=true, urlcolor=blue, linkcolor=black, colorlinks,citecolor=green]{hyperref}
...
\usepackage[nopostdot, nogroupskip, style=super, nonumberlist, toc]{glossaries}
\makeglossaries
...
Finally, we describe the acquisition functions and the aquisition optimization strategies used within the \gls{bananascp} setting.

\subsection{Acquisition Functions}
We consider four commonly used acquisition functions. 	Consistent with the notation in the previous sections, $\hat{F}_a$ represents the \gls{cdf} of the estimated distribution for architecture $a$. Then, depending on the acquisition function used, the specific acquisition score can be calculated as follows: 

\begin{description}[leftmargin=0cm] 
\item[\gls{its}:] A sample is drawn from the distribution at random and its value is seen as the acquistion score at the candidate architecture $a$.
\item[\gls{ucb}:] The acquisition score is given by $\mu+\gamma \cdot \sigma$ for a Gaussian distribution, where $\gamma$ is the exploration factor. For non-Gaussian distributions, we follow \cite{deshpande2024online} and generalize the function to a quantile function, i.e., $\hat{F}^{-1}_a(\gamma)$.  As in the original formulation, higher values of $\gamma$ promotes exploration. In our experiments, we set $\gamma=0.75$ due to these concerns: a) the distribution of model performance is believed to be left-skewed; b) estimates of extreme quantiles are generally based on sparse observations and therefore might be less reliable. A value of 0.75 offers a reasonable trade-off between exploration and accurate estimation.
\item[\gls{pi}:] The acquisition score corresponds to $1 - \hat{F}^{-1}_a(y_{max})$, with $y_{max}$ being the highest model performance ever observed.
...
\vspace{0.5cm}
  ...
  \begin{tabularx}{\textwidth}{lXlc}
    ...
    ChatGPT & Debugging LaTeX syntax errors  & Equation/Formula & + \\
    \bottomrule
  \end{tabularx}
\end{center}
