...
\noindent
\textbf{Definition (Conformal Prediction):}  Let \(B = \{(x_1, y_1), \dots, (x_n, y_n)\}\) be a bag of observed examples from an exchangeable joint distribution $\mathbb{P}_{X,Y}$, with $x_i \subset R^d$ representing some features and  $Y$ the target variable. Given a new unseen input \(x_{n+1}\), conformal prediction constructs a prediction region \( \mathcal{C}_n^\alpha(x_{n+1}) \) using a \textit{non-conformity score function} such that
\vspace{0.7em}
\[
\mathbb{P}\left( y_{n+1} \in \mathcal{C}_n^\alpha(x_{n+1}) \right) \geq 1 - \alpha
\vspace{0.7em}
\]

\noindent
where \( \alpha \in (0, 1) \) is the predefined nominal mis-coverage rate. This probability is also referred as the finite-sample guarantee property of \gls{cp} and is taken over the joint distribution of all \(n + 1\) samples. Moreover, the prediction regions for different $\alpha$ should be nested. That means, if  $\alpha_1 \ge \alpha_2$, then $1 - \alpha_1$ is a lower confidence level than $1 - \alpha_2$ and we have $ \; \mathcal{C}_n^{\alpha_1}(x_{n+1}) \subseteq \mathcal{C}_n^{\alpha_2}(x_{n+1})$.  

\vspace{0.7em}
\textbf{Non-conformity score function} is a real-valued function  measures how dissimilar the new example $x_{n+1}$ is compared to the existing examples in the bag $B$. Now generalizing the setting presented earlier to a supervised learning framework, a predictive algorithm $f$ that maps features to the target variable naturally leads to a non-conformity measure. Denote $\hat{f}_n$ the predictor trained on the $n$ observations in the bag for obtaining a point prediction for $x_{n+1}$. Since the prediction rule is learnt from all existing examples in the bag, intuitively, the distance between the predicted value $\hat{f}(x_{n+1})$ and the true value $y_{n+1}$ informs how different $(x_{n+1}, y_{n+1})$ is compared to the rest examples in the bag. As a result, the prediction set is constructed by collecting all values that lie within a certain distance from the true value.

A critical intuition behind constructing this prediction region is \textit{symmetry}, that is, all data points are treated symmetrically within the \gls{cp} framework. The exchangeability assumption implies that the rank of $y_{n+1}$ is uniformly distributed over the observed values $y_1,\, y_2, \, ..., \, y_n $. 




The finite-sample guarantee offered by the \gls{cp} framework is valid regardless of the size of the bag or the choice made for the underlying prediction algorithm or the non-conformity scores. However, the non-conformity score function is an important engineering decision and has 
...
(can add a figure illustrating why conformal prediction works, i.e., symmetry).

finite sample adjustment		
s	\item [Notation] 
	-  \\
	- A conformity score function s.  and has an impact on the size on the prediction set, i.e., the efficiency. The conformity score function can be either a negatively- or positively oriented, in which … And it can be a random variable as well. \\
